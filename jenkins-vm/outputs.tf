...
output "jenkins_access_info" {
  ...
  value       = "Jenkins is installed on ${google_compute_instance.jenkins_vm.name} at http://${google_compute_instance.jenkins_vm.network_interface[0].access_config[0].nat_ip}. Data is stored on the 20GB data disk mounted at /jenkins"
}
